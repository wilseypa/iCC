...
void Bi_Graph_Traversal::threadBackwardDFS(std::atomic<int>& reverted, int& iteration, int uidx) {
    ...
    // std::cout<< "iteration = "<<iteration<<" child idx = "<<uidx<<'\n';

    // std::vector<int> child_d_simp = std::move(getChild(uidx));
    // if (child_d_simp.size() == 0) return;
    ...
    while (!dfs_stack.empty()) {
        ...
        // std::cout<< "iteration = "<<iteration<<" top idx = "<<top<<'\n';

        std::vector<int> top_child = std::move(getChild(top));
        std::cout<<"top = "<<top<<"  top child = ";
        for (auto& t : top_child) {
            std::cout<<t<<"  ";}
        std::cout<<'\n';
...
int Bi_Graph_Traversal::parallelBFS(std::queue<int>& graph_bfs_queue, int threadnum, int iteration, int root) {
    ...
    while (!graph_bfs_queue.empty()) {
        ...
        auto remove_lambda = [&](const auto& i) {return visit_flag[i] > 0;};
        std::remove_if(front_child.begin(), front_child.end(), remove_lambda);

        //dynamic distribute work
        int worksize = front_child.size();

        std::cout<<"front = " <<front<<"  worksize = "<<worksize<<'\n';
        ...
        for (int i = 0; i < threadnum; i++) {
            thread_vec.emplace_back(work_lambda);
        }
