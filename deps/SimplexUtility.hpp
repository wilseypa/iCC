...
namespace SimplexUtility
{
    inline std::vector<std::vector<int64_t>> getBinomialTable(const size_t vtnum, const size_t maxdim)
    {
        ...
        for (size_t i = 1; i < vtnum + 1; i++)
        {
            ...
            {
                const int64_t upper_left = binom_table[i - 1][j - 1];
                const int64_t upper = binom_table[i - 1][j];
                if (upper_left > std::numeric_limits<int64_t>::max() - upper)
                    throw std::overflow_error("Binomial Overflow Error!");

                binom_table[i][j] = upper_left + upper;
            }
        }
...
#pragma omp parallel for schedule(dynamic) num_threads(worker_count)
    ...
    inline std::vector<int64_t> getFacetBinomialIndices(const std::vector<std::vector<int64_t>>& binomial_table, const int64_t bindex, const size_t dim)
    {
        ...
        for (size_t i = 0; i < simplex_vt.size(); i++)
        {
    ...
    inline void updateBinomialTable(std::vector<std::vector<int64_t>>& binomial_table, const size_t originalvtnum, const size_t pvnum, const size_t maxdim)
    {
        ...
        const size_t targetvtnum = originalvtnum + pvnum;

        if (currentvtnum >= targetvtnum)
            return;

        const size_t rows_to_add = targetvtnum - currentvtnum;
        ...
        for (size_t row = currentvtnum + 1; row <= targetvtnum; row++)
        {
            binomial_table[row][0] = 1;
            for (size_t j = 1; j < (maxdim + 1 + 1); j++)
            {
                const int64_t upper_left = binomial_table[row - 1][j - 1];
                const int64_t upper = binomial_table[row - 1][j];
                if (upper_left > std::numeric_limits<int64_t>::max() - upper)
                    throw std::overflow_error("Binomial Overflow Error!");

                binomial_table[row][j] = upper_left + upper;
            }
        }
    ...
    /**
     * Visits every immediate cofacet in increasing resulting binomial-index order.
     * The candidate action receives (co-vertex, cofacet bindex) and returns true
     * when the desired candidate was found, or false to continue the sweep.
     */
    template <typename CofacetCandidateAction>
    inline void forEachImmediateCofacetInBindexOrder(
        const std::vector<std::vector<int64_t>>& binomial_table,
        std::vector<size_t>& facet_vertices_workspace,
        const int64_t facetbidx,
        const size_t total_label_ct,
        const size_t facetdim,
        CofacetCandidateAction&& process_cofacet_candidate)
    {
        ...
        // Co-vertices below the minimum facet vertex occupy slot 0; all facet vertices are shifted.
        for (size_t covt = 0; covt < facet_vertices_workspace.back(); ++covt)
        {
            if (process_cofacet_candidate(covt, base + binomial_table[covt][1]))
                return;
        }
        ...
        for (size_t i = m - 1; i > 0; --i)
        {
            ...
            {
                if (process_cofacet_candidate(covt, base + binomial_table[covt][pos + 1 + 1]))
                    return;
            }
        }
        ...
        {
            if (process_cofacet_candidate(covt, base + binomial_table[covt][m + 1]))
                return;
        }
    }
                                       ...
                                       std::vector<size_t>& facet_indices, std::vector<size_t>& cofacet_vertices_workspace, int64_t cofacetbindex, size_t npts, size_t cofacetdim)
    {
        ...
        for (size_t i = 0; i < cofacet_vertices_workspace.size(); i++)
        {
